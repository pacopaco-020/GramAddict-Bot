...
# Dedicated VPN IP
RUN_COUNT_DIR="/Users/milan/Documents/bots/gramaddict/bot/run_counts_2"
...
user_order=( "area.nl" "weareedn_amsterdam" "alphabetstreet.marketing" )
MAX_RUNS_PER_DAY=4

# Use Python 3.9 for compatibility with GramAddict 3.2.12
...
while true; do
    ...
    if [ "$current_hour" -ge 7 ] && [ "$current_hour" -lt 22 ]; then
        ...
        # Pre-session health check and UIAutomator recovery
        echo "Performing pre-session health check..."
        
        # Check device responsiveness
        if ! check_device_connection; then
            echo "Device not responsive during health check. Attempting device restart..."
            if restart_device; then
                sleep 60  # Wait for device to fully boot
            else
                echo "Device restart failed during health check. Skipping session."
                increment_error_count
                continue
            fi
        fi
