...
def _stringify_interactions(interactions):
    ...
    # Handle cases where interactions might not be a proper dict
    if not isinstance(interactions, dict):
        return "0"

    result = ""
    for source, count in interactions.items():
        # Handle None values safely with extra validation
        try:
            source_str = str(source) if source is not None else "unknown"
            count_str = str(count) if count is not None else "0"
            result += count_str + " for " + source_str + ", "
        except (TypeError, ValueError) as e:
            logger.warning(f"Error processing interaction data: source={source}, count={count}, error={e}")
            result += "0 for unknown, "
    
    result = result[:-2] if result else "0"
    return result
