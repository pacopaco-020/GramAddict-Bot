...
from Instamatic.core.device_facade import (
    DeviceFacade,
    Direction,
    Location,
    Mode,
...
from Instamatic.core.resources import ClassName
from Instamatic.core.resources import ResourceID as resources
from Instamatic.core.resources import TabBarText
    ...
    def _getActionBar(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "AppHasCrashed" in str(e) and attempt < max_retries - 1:
                    logger.warning(f"ActionBar initialization attempt {attempt+1} failed, Instagram may be loading. Retrying...")
                    sleep(2)
                    continue
                elif attempt == max_retries - 1:
                    logger.warning(f"ActionBar initialization failed after {max_retries} attempts. Returning None.")
...
class HomeView(ActionBarView):
    ...
    def navigateToSearch(self):
        ...
        # Try to find search button with fallback methods
        search_btn = self.action_bar.child(
            descriptionMatches=case_insensitive_re(TabBarText.SEARCH_CONTENT_DESC)
        )
        
        if not search_btn.exists():
            # Fallback: try direct search without action_bar
            logger.debug("Search button not found in action_bar, trying direct search")
            search_btn = self.device.find(
                descriptionMatches=case_insensitive_re(TabBarText.SEARCH_CONTENT_DESC)
            )
        ...
        if search_btn.exists():
            ...
            logger.error("Could not find Search tab - Instagram UI may have changed")
            
            # Try multiple fallback methods
            fallback_methods = [
                # Method 1: Try by resource ID
                lambda: self.device.find(resourceIdMatches=case_insensitive_re(".*search.*")),
                # Method 2: Try by text content  
                lambda: self.device.find(textMatches=case_insensitive_re(".*search.*")),
                # Method 3: Try action bar search
                lambda: self.device.find(resourceIdMatches=case_insensitive_re(".*action_bar.*search.*")),
                # Method 4: Try direct search via key event
                lambda: self._try_search_key_event()
            ]
            
            for i, method in enumerate(fallback_methods):
                try:
                    if i < 3:  # First 3 methods return UI elements
                        search_btn = method()
                        if search_btn and search_btn.exists():
                            search_btn.click()
                            sleep(2)
                            return SearchView(self.device)
                    else:  # Last method is key event
                        if method():
                            return SearchView(self.device)
                except Exception as e:
                    logger.warning(f"Search fallback method {i+1} failed: {e}")
                    continue
            
            # Try to navigate home first, then retry
            self.device.back()
            sleep(2)
            search_btn = self.device.find(
                descriptionMatches=case_insensitive_re(TabBarText.SEARCH_CONTENT_DESC)
            ...
            if search_btn.exists():
                ...
                raise Exception("Could not find Search tab after all retry methods")

        return SearchView(self.device)
    
    def _try_search_key_event(self):
        """Try to open search using key event as last resort"""
        try:
            self.device.press_key("search")
            sleep(2)
            return True
        except Exception as e:
            logger.warning(f"Search key event failed: {e}")
            return False
...
class AccountView:
    ...
    def navigate_to_main_account(self):
        ...
        try:
            ...
            # Create a dummy view for coordinate clicking
            dummy_view = self.device.find(text="")
            dummy_view.click(mode=Location.CUSTOM, coord=[profile_x, profile_y])
            random_sleep(1, 2)
    ...
    def refresh_account(self):
        ...
        while retry_count < max_retries:
            try:
                ...
                # If we reach here, refresh was successful
                logger.info("Account refresh completed successfully")
                return
                
            except DeviceFacade.AppHasCrashed:
                retry_count += 1
                logger.warning(f"App crashed during refresh_account, attempt {retry_count}/{max_retries}")
                
                if retry_count < max_retries:
                    logger.info("Waiting for Instagram to stabilize after crash...")
                    time.sleep(5)  # Wait a bit for app to stabilize
                    
                    # Try to verify Instagram is actually running
                    if not self.device._ig_is_opened():
                        logger.info("Instagram not detected as open, waiting longer...")
                        time.sleep(10)
                else:
        ...
        if post_media_view.exists(Timeout.MEDIUM):
            ...
            while True:
                try:
                    ...
                    logger.warning(f"UIAutomator2 error while finding like button (attempt {attempt + 1}/4): {e}")
                    if attempt == 3:
                        logger.error("Failed to find like button after all retries due to UIAutomator2 issues")
                        return None
                    time.sleep(2)
                except Exception as e:
                    logger.error(f"Unexpected error while finding like button: {e}")
                    return None
                ...
                try:
                    ...
                    logger.warning(f"Error during swipe while finding like button: {e}")
            ...
            if self.has_tags:
                ...
                if like_button is not None:
                    try:
                        ...
                        logger.warning(f"Error clicking like button: {e}")
                        logger.info("Trying double-click on post instead")
                        try:
                            post_media_view.double_click()
                            liked, like_button = self._is_post_liked()
                        except Exception as e2:
                            logger.error(f"Failed to like post with double-click fallback: {e2}")
                            return False
                else:
                    logger.warning("Can't find the like button object! Trying double-click instead.")
                    try:
                        post_media_view.double_click()
                        liked, like_button = self._is_post_liked()
                    except Exception as e:
                        logger.error(f"Failed to like post with double-click: {e}")
                        return False
            else:
                try:
                    post_media_view.double_click()
...
class ProfileView(ActionBarView):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.debug(f"Action bar not found, trying alternative detection (attempt {attempt+1}/{max_retries})")
                    sleep(2)
                    
                    # Try finding by text content
                    text_based_bar = self.device.find(textMatches=".*")
                    if text_based_bar.exists(Timeout.SHORT):
                        # Check if any text element looks like a username (contains @ or is at top of screen)
                        try:
                            bounds = text_based_bar.get_bounds()
                            if bounds and bounds.get("top", 0) < 200:  # Likely in action bar area
                                return text_based_bar
                        except:
                            pass
                    continue
                else:
                    logger.warning("Unable to find action bar after retries! (The element with the username at top)")
                    return None
            except Exception as e:
                if attempt < max_retries - 1:
                    logger.debug(f"Action bar detection failed (attempt {attempt+1}), retrying: {str(e)}")
                    sleep(2)
                    continue
                else:
                    logger.error(f"Action bar detection failed after {max_retries} attempts: {str(e)}")
                    return None
    ...
    def getUsername(self, watching_stories=False):
        ...
        for attempt in range(max_retries):
            ...
            if action_bar is not None:
                try:
                    username = action_bar.get_text(error=not watching_stories).strip()
                    if username:
                        return username
                except Exception as e:
                    logger.warning(f"Failed to get text from action bar (attempt {attempt+1}): {str(e)}")
                    if attempt < max_retries - 1:
                        sleep(2)
                        continue
            
            # Fallback: try multiple resource IDs for username
            fallback_ids = [
                ResourceID.USERNAME_TEXTVIEW,
                ResourceID.ACTION_BAR_TITLE,
                ResourceID.TITLE_VIEW
            ]
            for fallback_id in fallback_ids:
                try:
                    profile_header = self.device.find(resourceId=fallback_id)
                    if profile_header.exists(Timeout.SHORT):
                        username = profile_header.get_text(error=False).strip()
                        if username:
                            logger.info(f"Username found via fallback ID: {fallback_id}")
                            return username
                except Exception as e:
                    logger.debug(f"Fallback detection failed for {fallback_id}: {str(e)}")
            
            if attempt < max_retries - 1:
                logger.warning(f"Username detection failed (attempt {attempt+1}), retrying...")
                sleep(3)
            else:
                logger.error("Cannot get username from any method after retries.")
                return None
            ...
            try:
                ...
                if followers_text_view.exists(Timeout.MEDIUM):
                    ...
                    if followers_text:
                        ...
                        logger.debug(f"Cannot get followers count text (attempt {attempt+1}).")
                else:
                    logger.debug(f"Cannot find followers count view (attempt {attempt+1}).")
                
                if attempt < max_retries - 1:
                    logger.debug(f"Retrying followers count detection in 2 seconds...")
                    sleep(2)
            except Exception as e:
                logger.debug(f"Followers count detection error (attempt {attempt+1}): {str(e)}")
                if attempt < max_retries - 1:
                    sleep(2)

        logger.error("Cannot get followers count after retries.")
        return None
            ...
            try:
                ...
                if following_text_view.exists(Timeout.MEDIUM):
                    ...
                    if following_text:
                        ...
                        logger.debug(f"Cannot get following count text (attempt {attempt+1}).")
                else:
                    logger.debug(f"Cannot find following count view (attempt {attempt+1}).")
                
                if attempt < max_retries - 1:
                    logger.debug(f"Retrying following count detection in 2 seconds...")
                    sleep(2)
            except Exception as e:
                logger.debug(f"Following count detection error (attempt {attempt+1}): {str(e)}")
                if attempt < max_retries - 1:
                    sleep(2)
            ...
            try:
                ...
                if post_count_view.exists(Timeout.MEDIUM):
                    ...
                    if count is not None:
                        parsed_count = self._parseCounter(count)
                        if parsed_count is not None:
                            return parsed_count
                    else:
                        logger.debug(f"Cannot get posts count text (attempt {attempt+1}).")
                else:
                    logger.debug(f"Cannot find posts count view (attempt {attempt+1}).")
                
                if attempt < max_retries - 1:
                    logger.debug(f"Retrying posts count detection in 2 seconds...")
                    sleep(2)
            except Exception as e:
                logger.debug(f"Posts count detection error (attempt {attempt+1}): {str(e)}")
                if attempt < max_retries - 1:
                    sleep(2)
